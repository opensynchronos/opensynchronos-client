...
using NotificationType = OpenSynchronos.MareConfiguration.Models.NotificationType;

namespace OpenSynchronos.Services;
...
public class NotificationService : DisposableMediatorSubscriberBase, IHostedService
{
    ...
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[OpenSynchronos] Error: " + message);
        _chatGui.PrintError(se.BuiltString);
    }

    private void PrintInfoChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[OpenSynchronos] Info: ").AddItalics(message ?? string.Empty);
        _chatGui.Print(se.BuiltString);
    }

    private void PrintWarnChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[OpenSynchronos] ").AddUiForeground("Warning: " + (message ?? string.Empty), 31).AddUiForegroundOff();
        _chatGui.Print(se.BuiltString);
    }
