...
public sealed class DtrEntry : IDisposable, IHostedService
{
    ...
    private IDtrBarEntry CreateEntry()
    {
        ...
        var entry = _dtrBar.Get("OpenSynchronos");
    ...
    private void Update()
    {
        ...
        if (_apiController.IsConnected)
        {
            ...
            if (pairCount > 0)
            {
                ...
                tooltip = $"OpenSynchronos: Connected{Environment.NewLine}----------{Environment.NewLine}{string.Join(Environment.NewLine, visiblePairs)}";
                colors = _configService.Current.DtrColorsPairsInRange;
            }
            else
            {
                tooltip = "OpenSynchronos: Connected";
        ...
        {
            text = "\uE044 \uE04C";
            tooltip = "OpenSynchronos: Not Connected";
