...
public class CensusPopupHandler : IPopupHandler
{
    ...
    public void DrawContent()
    {
        ...
        {
            start = ImGui.GetCursorPosY() - ImGui.CalcTextSize("OpenSynchronos Census Data").Y;
            UiSharedService.TextWrapped("OpenSynchronos Census Participation");
        }
        ImGuiHelpers.ScaledDummy(5f);
        UiSharedService.TextWrapped("If you are seeing this popup you are updating from a OpenSynchronos version that did not collect census data. Please read the following carefully.");
        ImGui.Separator();
        UiSharedService.TextWrapped("OpenSynchronos Census is a data collecting service that can be used for statistical purposes. " +
            "All data collected through OpenSynchronos Census is temporary and will be stored associated with your UID on the connected service as long as you are connected. " +
            "The data cannot be used for long term tracking of individuals.");
        UiSharedService.TextWrapped("If enabled, OpenSynchronos Census will collect following data:" + Environment.NewLine
