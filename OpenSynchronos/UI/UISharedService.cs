...
using OpenSynchronos.Localization;
...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
    ...
    {
        ColorTextWrapped("Note: The storage folder should be somewhere close to root (i.e. C:\\OpenSynchronosStorage) in a new empty folder. DO NOT point this to your game folder. DO NOT point this to your Penumbra folder.", ImGuiColors.DalamudYellow);
        ...
        using (ImRaii.Disabled(_cacheMonitor.MareWatcher != null))
        {
            ...
            {
                FileDialogManager.OpenFolderDialog("Pick OpenSynchronos Storage Folder", (success, path) =>
                {
                    if (!success) return;
                    ...
                    {
                        _cacheDirectoryHasOtherFilesThanCache = true;
                        Logger.LogWarning("Found folders in {path} not belonging to OpenSynchronos: {dirs}", path, string.Join(", ", dirs));
                    }
        ...
        {
            ColorTextWrapped("Your selected directory has files or directories inside that are not OpenSynchronos related. Use an empty directory or a previous OpenSynchronos storage directory only.", ImGuiColors.DalamudRed);
        }
        ...
        DrawHelpText("The storage is automatically governed by OpenSynchronos. It will clear itself automatically once it reaches the set capacity by removing the oldest unused files. You typically do not need to clear it yourself.");
    }
