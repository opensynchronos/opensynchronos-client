...
using OpenSynchronos.API.Data.Comparer;
using OpenSynchronos.API.Routes;
...
using OpenSynchronos.PlayerData.Handlers;
...
using OpenSynchronos.WebAPI.Files;
...
public class SettingsUi : WindowMediatorSubscriberBase
{
    ...
    private void DrawBlockedTransfers()
    {
        ...
        UiSharedService.ColorTextWrapped("Files that you attempted to upload or download that were forbidden to be transferred by their creators will appear here. " +
                             ...
                             "Ask your paired friend to send you the mod in question through other means, acquire the mod yourself or ask the mod creator to allow it to be sent over OpenSynchronos.",
    ...
    private void DrawFileStorageSettings()
    {
        ...
        if (_uiShared.IconTextButton(FontAwesomeIcon.Running, "Open OpenSynchronos Character Data Hub"))
        {
        ...
        UiSharedService.TextWrapped("OpenSynchronos stores downloaded files from paired people permanently. This is to improve loading performance and requiring less downloads. " +
        ...
        ImGui.AlignTextToFramePadding();
        ImGui.TextUnformatted("Monitoring OpenSynchronos Storage Folder: " + (_cacheMonitor.MareWatcher?.Path ?? "Not monitoring"));
        ...
        if (_cacheMonitor.MareWatcher == null || _cacheMonitor.PenumbraWatcher == null)
        {
            ...
            UiSharedService.AttachToolTip("Attempts to resume monitoring for both Penumbra and OpenSynchronos Storage. "
        ...
        else
        {
            ...
            UiSharedService.AttachToolTip("Stops the monitoring for both Penumbra and OpenSynchronos Storage. "
                + "Do not stop the monitoring, unless you plan to move the Penumbra and OpenSynchronos Storage folders, to ensure correct functionality of OpenSynchronos." + Environment.NewLine
        ...
        {
            UiSharedService.ColorTextWrapped("Hint: To free up space when using OpenSynchronos consider enabling the File Compactor", ImGuiColors.DalamudYellow);
        }
        ...
        if (!_fileCompactor.MassCompactRunning)
        {
            ...
            UiSharedService.AttachToolTip("This will run compression on all files in your current OpenSynchronos Storage." + Environment.NewLine
            ...
            UiSharedService.AttachToolTip("This will run decompression on all files in your current OpenSynchronos Storage.");
        }
        else
        {
        ...
        ImGui.Separator();
        UiSharedService.TextWrapped("File Storage validation can make sure that all files in your local OpenSynchronos Storage are valid. " +
    ...
    private void DrawGeneral()
    {
        ...
        _uiShared.DrawHelpText("This will add OpenSynchronos related right click menu entries in the game UI on paired players.");
        ...
        _uiShared.DrawHelpText("This will add OpenSynchronos connection status and visible pair count in the Server Info Bar.\nYou can further configure this through your Dalamud Settings.");
        ...
        if (ImGui.Checkbox("Show OpenSynchronos Profiles on Hover", ref showProfiles))
        {
    ...
    private void DrawPerformance()
    {
        ...
        _uiShared.DrawHelpText("OpenSynchronos will print a warning in chat once per session of meeting those people. Will not warn on players with preferred permissions.");
        using (ImRaii.Disabled(!warnOnExceedingThresholds && !showPerformanceIndicator))
        {
            ...
            _uiShared.DrawHelpText("OpenSynchronos will also print warnings and show performance indicator for players where you enabled preferred permissions. If warning in general is disabled, this will not produce any warnings.");
        }
    ...
    private void DrawServerConfiguration()
    {
        ...
        if (ImGui.BeginTabBar("serverTabBar"))
        {
            if (ImGui.BeginTabItem("Character Management"))
            {
                ...
                {
                    UiSharedService.ColorTextWrapped("Characters listed here will automatically connect to the selected OpenSynchronos service with the settings as provided below." +
                    ...
                    foreach (var item in selectedServer.Authentications.ToList())
                    {
                        ...
                        if (ImGui.Checkbox("Automatically login to OpenSynchronos", ref isAutoLogin))
                        {
                            item.AutoLogin = isAutoLogin;
                            _serverConfigurationManager.Save();
                        }
                        _uiShared.DrawHelpText("When enabled and logging into this character in XIV, OpenSynchronos will automatically connect to the current service.");
            ...
            if (ImGui.BeginTabItem("Service Configuration"))
            {
                ...
                if (_dalamudUtilService.IsWine)
                {
                    ...
                    _uiShared.DrawHelpText("On wine, OpenSynchronos will automatically fall back to ServerSentEvents/LongPolling, even if WebSockets is selected. "
    ...
    private async Task<(bool Success, bool partialSuccess, string Result)> ConvertSecretKeysToUIDs(ServerStorage serverStorage, CancellationToken token)
    {
        ...
        var oauthCheckUri = OpenSynchronosAuth.GetUIDsBasedOnSecretKeyFullPath(new Uri(baseUri));
    ...
    private void DrawSettingsContent()
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button("OpenSynchronos Discord"))
        {
