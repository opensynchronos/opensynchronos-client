...
namespace OpenSynchronos.Interop.Ipc;
...
public sealed class IpcCallerPenumbra : DisposableMediatorSubscriberBase, IIpcCaller
{
        ...
        finally
        {
            if (!penumbraAvailable && !_shownPenumbraUnavailable)
            {
                ...
                _mareMediator.Publish(new NotificationMessage("Penumbra inactive",
                    "Your Penumbra installation is not active or out of date. Update Penumbra and/or the Enable Mods setting in Penumbra to continue to use OpenSynchronos. If you just updated Penumbra, ignore this message.",
    ...
    public async Task<Guid> CreateTemporaryCollectionAsync(ILogger logger, string uid)
    {
        ...
        return await _dalamudUtil.RunOnFrameworkThread(() =>
        {
            ...
            _penumbraCreateNamedTemporaryCollection.Invoke("OpenSynchronos", collName, out Guid collId);
